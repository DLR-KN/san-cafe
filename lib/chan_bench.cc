...
//#define TAP_FILE
//"/home/jkraemer/Development/DLR/cuda-pfb/config/prot_filter_45x2.dat"
#define TAP_FILE "../config/prot_filter_45x2.dat"

static void mallocOutBufferVector(vector_void_star &outVec, uint32_t vecSize,
                                  uint32_t size, size_t alignment)
{
  complex_float *tmpPtr;
  for (unsigned int i = 0; i < vecSize; ++i) {
    tmpPtr =
        (complex_float *)arch_malloc(size * sizeof(complex_float), alignment);
    outVec.push_back(tmpPtr);
  }
  tmpPtr = nullptr;
}

static void freeOutBufferVector(vector_void_star &outVec)
{
  for (auto i : outVec) {
    free(i);
  }
...
static void condition_input(complex_float *in, int numChannels,
...
int main()
{
  ...
  mallocOutBufferVector(outVec, numFilters,
                        numSamples / numFilters * oversampling, alignment);
  ...
  gr::filter::pfb_channelizer_ccf::sptr chan =
      gr::filter::pfb_channelizer_ccf::make(numFilters, taps, oversampling);
  ...
  for (unsigned int i = 0; i < numRuns; ++i) {
    ...
    producedSamples = chan->general_work(numSamples / numFilters * oversampling,
                                         numIn, inVec, outVec);
  ...
  writeComplexSamplesToFile((complex_float *)outVec[0], producedSamples,
                            "outSamples0.dat");
  writeComplexSamplesToFile((complex_float *)outVec[1], producedSamples,
                            "outSamples1.dat");
  writeComplexSamplesToFile((complex_float *)outVec[2], producedSamples,
                            "outSamples2.dat");
  writeComplexSamplesToFile((complex_float *)outVec[5], producedSamples,
                            "outSamples5.dat");
  writeComplexSamplesToFile((complex_float *)outVec[7], producedSamples,
                            "outSamples7.dat");
  writeComplexSamplesToFile((complex_float *)outVec[10], producedSamples,
                            "outSamples10.dat");
  writeComplexSamplesToFile((complex_float *)outVec[30], producedSamples,
                            "outSamples30.dat");
